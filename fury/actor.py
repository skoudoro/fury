...
def superquadric_slicer(evals, evecs, affine=None, mask=None, sphere=None,
                        scale=2.2, norm=True, opacity=1., scalar_colors=None):
    if not evals.shape == evecs.shape[:-1]:
        msg = "Eigenvalues shape {} is not compatible ".format(evals.shape)
        msg += "with eigenvectors' {}. ".format(evecs.shape)
        msg += "Please provide eigenvalue and eigenvector arrays that have "
...
def _superquadric_slicer_mapper(evals, evecs, affine=None, mask=None,
                                sphere=None, scale=2.2, norm=True,
                                scalar_colors=None, fa=None, md=None):
    ...
    print("FA, MD", fa, md)
    for (k, center) in enumerate(ijk):
        faces, vertices = fp.prim_superquadric((fa, md))
        ea = evals[tuple(center.astype(np.int))]
        # print("faces", faces)
        if norm:
            ea /= ea.max()
